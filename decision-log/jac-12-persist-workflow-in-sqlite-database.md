...
### Frontend wiring: a fixed client-generated `lid`, not a fixed server `id`

- **Choice:** The editor only ever works on one workflow (no picker yet), so it needs a stable identity across page reloads. Rather than the frontend dictating a hardcoded server `id` — which would require `POST` to honor a client-supplied id, reversing the "server always assigns" decision above — the `Workflow` model gained a second, purely-passthrough field: `lid` (local id). The frontend sends a fixed constant (`workflow-editor-demo`) as `lid` on create; the server ignores it for id assignment but stores and echoes it back. The frontend then caches the *server-returned* `id` in `localStorage` (`workflow-editor:workflow-id`) and uses that for all subsequent `GET`/`PUT` calls. On load: read the cached id → `GET` it; if there's no cached id, or the `GET` 404s (e.g. the SQLite file was reset), fall back to `POST`-ing the seed graph with `lid` set, cache the id it comes back with, and go from there.
- **Alternatives considered:** Let `POST` honor a client-supplied `id` directly (rejected — reverses an already-settled decision and conflates "the id the client would like" with "the id the server guarantees is unique"). Look up existing workflows by `lid` server-side, upsert-style (rejected as unnecessary for this ticket's scope — see follow-up below).
- **Trade-offs / risks accepted:** There's no server-side dedup by `lid`. If `localStorage` is cleared (or a second browser/tab loads before the first ever saves), the fallback path creates a *new* row with the same `lid`, not the original one — so today `lid` is stored but not yet load-bearing for anything. It exists so a future ticket can add "find-or-create by `lid`" without another schema change. `save()` always re-sends the fixed `lid` alongside the current graph, since `PUT` replaces the whole stored payload (see the decision above) and would otherwise silently null it out on the first save.

### Mapping between ReactFlow's node/edge shape and the persisted `Workflow`

- **Choice:** Added `entities/workflow/model/mapping.ts` (`flowToWorkflow` / `workflowToFlow`), since the two shapes differ (ReactFlow's node `type` is always the renderer key `"workflowNode"`; the domain `NodeType` lives at `data.nodeType` — and edges use `source`/`sourceHandle` vs. `sourceNodeId`/`sourceHandleId`). This was flagged as an open exercise question in `flowTypes.ts`'s own docstring, so it's now resolved.
- **Trade-offs / risks accepted:** A `FlowEdge` with a missing `sourceHandle`/`targetHandle` (ReactFlow allows this) maps to an empty string, not `null` — `WorkflowEdge`'s handle-id fields are non-optional strings on the backend. Not currently reachable through the UI (every node's handles are pre-populated by `nodeFactory`), but worth a second look if that ever changes.
...
- The frontend now calls `createWorkflow`/`getWorkflow`/`updateWorkflow` from a single `useWorkflowPersistence` hook (`features/persist-workflow`), wired into `WorkflowEditorPage` — load-or-create on mount, a basic Save button for `updateWorkflow`. This closes the "no live UI caller yet" gap noted below previously.
- A real multi-workflow editor (a picker, per-document `lid`s, dedup-by-`lid` on the backend) is out of scope here — today's `lid` is a placeholder for that, not a working implementation of it.
- Verified end-to-end manually against a running server and a real SQLite file (POST → GET → PUT → GET, plus the 404 and 422 error shapes, plus the new frontend flow: create-on-first-load, reload-uses-GET-not-POST, Save persists edits and preserves `lid`) in addition to the automated test suite.
